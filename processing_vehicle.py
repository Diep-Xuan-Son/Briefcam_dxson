...
def load_network(network):
  save_path = os.path.join('./weights', 'vehicle.pth')
...
if __name__ == '__main__':
  ...
  with open('./doc/car_final_label.json', 'r') as labels:
  ...
  while True:
    ...
    if ret:
      ...
      img = frame
      h, w = img.shape[:2]
      dets = choose_class(names, dets, ['car', 'bus'])
      for i in dets:
      ...
      for i in range(len(trackers)):
        ...
        if (x2-x1) > 50 and (y2 -y1) > 50:
          att_all[ids] = torch.cat((att_all[ids], pred), 0)
  ...
  result['data'] = data_img
  result['time'] = time_result
  torch.save(result, f'/content/result/{basename}.pt')